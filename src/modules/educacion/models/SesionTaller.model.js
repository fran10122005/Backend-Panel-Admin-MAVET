...
const SesionTaller = sequelize.define(
  'SesionTaller',
  {
    id_sesion: { type: DataTypes.STRING(15), primaryKey: true },
    id_taller: { type: DataTypes.STRING(15) },
    fecha: { type: DataTypes.DATEONLY },
    tema_impartido: { type: DataTypes.STRING(255) },
  },
  {
    tableName: 'sesiones_talleres',
    hooks: {
      beforeCreate: async (instance, options) => {
        ...
        let newNumber = 1;
        if (lastRecord && lastRecord[pkField] && lastRecord[pkField].startsWith('STA-')) {
          const lastNumber = parseInt(lastRecord[pkField].replace('STA-', ''), 10);
          if (!isNaN(lastNumber)) {
            newNumber = lastNumber + 1;
          }
        }

        instance[pkField] = `STA-${String(newNumber).padStart(5, '0')}`;
      },
    },
    timestamps: false,
  }
